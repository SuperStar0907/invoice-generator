...
class InvoiceForm extends React.Component {
  constructor(props) {
    ...
    this.state = this.props.location.state ? this.props.location.state : {
      ...
      currency: '$',
      currentDate: '',
      invoiceNumber: this.props.invoices.invoices.length + 1,
      dateOfIssue: '',
      billTo: '',
      billToEmail: '',
      billToAddress: '',
  ...
  componentDidMount(prevProps) {
    if (this.props.location.state?.copymode || !this.props.location.state) {
      const maxInvoiceNumber = Math.max(...this.props.invoices.invoices.map((invoice) => invoice.invoiceNumber));
      this.setState({
        invoiceNumber: maxInvoiceNumber > 0 ? maxInvoiceNumber + 1 : 1
      });
    }
