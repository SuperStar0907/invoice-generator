...
class InvoiceModal extends React.Component {
  handleAddInvoice = () => {
    const { addInvoice, info } = this.props;
    addInvoice(info);
    this.props.navigate('/');
  }
  handleEditInvoice = () => {
    const { editInvoice, info } = this.props;
    editInvoice(info);
    this.props.navigate('/');
  }
  handleSubmit = () => {
    if(this.props.invoices.invoices.length>=this.props.info.invoiceNumber) {
      ...
      <div>
        <Modal show={this.props.showModal} onHide={this.props.closeModal} size="lg" centered>
          ...
          <div className="pb-4 px-4">
            <Row>
              <Col md={6}>
                <Button variant="primary" className="d-block w-100" onClick={this.handleSubmit}>
...
const mapDispatchToProps = { addInvoice,editInvoice };

export default connect(mapStateToProps, mapDispatchToProps)(withRouter(InvoiceModal));
